...
class Tree(sg.Tree):
    ...
    def load_tree(self, dictionary):
        ...
        while children != []:
            ...
            for child in children:
                ...
                self.treedata.Insert(table[node[0]], table[child], node[2], node[3])
                temp += node[1]
            children = temp
        self.tree.update(values=self.treedata)
    ...
    def _key_to_id(self, key):
        ...
        """
        return [
            k for k in self.tree.IdToKey if (self.tree.IdToKey[k] == key)][0]
