...
class Button(sg.Button):
    ...
    def __init__(
        ...
        bind_return_key=False, metadata=None):
        """
        ...
        No Image allowed here and button_color must be None or
            ('#rrggbb', '#rrggbb').

        : Parameters - Please refer to sg.Button
        : Return Instance of new Button class
        """
        data = self._image(font, button_color, size)
        if button_color:
    ...
    def _hex_to_tuple(self, string):
        """
        Convert '#rrggbb' to (r, g, b, 255)
        : Parameter
          string - str, color '#rrggbb'.
        : return
          (r, g, b, 255) all in int 0 ~ 255.
        """
        return tuple(bytes.fromhex(string[1:])) + (255,)

    def _image(self, font, button_color, size):
        """
        ...
        """
        color = button_color if button_color else ('#ffffff', '#0000ff')
        s1 = size[0] if size[0]!=None else sg.DEFAULT_BUTTON_ELEMENT_SIZE[0]
        text, background = map(self._hex_to_tuple, color)
        font = font if font else sg.DEFAULT_FONT
        family, s2, style = self._font(font)  # style ignored
        width, height = s1*s2, s2*3
        radius = height//2
        im = Image.new(
